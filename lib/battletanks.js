...
var BattleTank = function() {
	...
	var Tank = function(tankId, x, y, color, grid) {
		...
		var CPU = function() {
			// mailboxes for LMC
			this.ram = [];
			this.registers = {accumulator:0, programCounter:0}
			...
			this.load = function(code) {
				...
				for(var i = 0; i < lines.length; i++) {
					src = lines[i].trim();
					line = src.match(/^(([A-Za-z_]+)\s+)?(SUB|ADD|STA|LDA|BRA|BRZ|BRP|INP|OUT|HLT|DAT)?(\s+([A-Za-z_0-9]+))?(\s*\/\/\s*(.*))?$/);
					if(line) {
				...
				if(result.success) {
					for(var i = 0; i < result.instructions.length; i++) {
						...
						if(/[A-Za-z]+/.test(result.instructions[i].address)) {
							result.instructions[i].addressLabel = result.instructions[i].address;
							result.instructions[i].address = result.labels[result.instructions[i].address];
						}
						
						cmd = result.instructions[i].opcode;
						if(result.instructions[i].address) {
							cmd += " " + result.instructions[i].address;
						}
		...
		this.showCPU = function() {
			...
			var html = '<div id="ram"><h2>Little Man Computer</h2>Each tank is controlled by a processor called a Little Man Computer (LMC). A LMC has 100 different memory locations. Each memory location can store a number from -999 to 999. Each line of code that you write is assembled into a number that is stored in one memory address.';
			
			
			html += '<div><div id="ram_editor_holder"><h2>Code editor</h2><a data-icon="gear" data-inline="true" data-mini="true" id="btn_editor_assemble">Assemble</a><a data-icon="delete" data-inline="true" data-mini="true" id="btn_editor_cancel">Cancel</a><div id="ram_editor">' + code + '</div><span id="code_status">Little Man Computer</span></div>'
			html +='<div id="ram_contents">Memory contents'
			...
			var checkCode = function() {
				...
				checkTimeout = setTimeout(function(e) {
					...
					if(result.success) {
						success = true;
						$("#code_status").html('<img src="media/tick.png" />No errors: ' + result.instructions.length + " / 100 memory used").css({'background-color':'rgba(200,255,200,0.7)', 'box-shadow':'0px 0px 10px rgba(200,255,200,0.7)'});
						
						var html ='Memory contents';
						for(var i = 0; i < tank.cpu.ram.length; i++) {
							html += '<div class="memory_location" id="memory_location_' + i + '"><span id="memory_address_' + i + '" class="memory_address">' + tank.pad(i,2) + '</span><span id="memory_value_' + i + '" class="memory_value">' + tank.pad(tank.cpu.ram[i], 3) + '</span></div>';
						}
